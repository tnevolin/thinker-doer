...
void __cdecl modified_enemy_units_check(int factionId)
{
	...
	if (isWtpEnabledFaction(factionId))
	{
		...
		for (int vehicleId = 0; vehicleId < *total_num_vehicles; vehicleId++)
		{
			...
			if (vehicle->road_moves_spent >= speed)
				continue;
			
			vehicle->road_moves_spent = speed;
...
void populateAssociations()
{
	...
	for (int tileIndex = 0; tileIndex < *map_area_tiles; tileIndex++)
	{
		...
		if (tileIndex == *map_half_x)
		{
			tileIndex += *map_half_x * (*map_axis_y - 2);
		}
	...
	for (int factionId = 0; factionId < MaxPlayerNum; factionId++)
	{
		...
		for (int tileIndex = 0; tileIndex < *map_area_tiles; tileIndex++)
		{
			...
			{
				int polarRegionIndex = (tileIndex < *map_area_tiles / 2 ? tileIndex : *map_half_x + tileIndex - (*map_area_tiles - *map_half_x));
		...
		for (robin_hood::pair<int, int> &extendedRegionAssociationEntry : extendedRegionAssociations)
		{
			...
			else
			{
				...
				int tileIndex = (polarRegionIndex < *map_half_x ? polarRegionIndex : polarRegionIndex - *map_half_x + (*map_area_tiles - *map_half_x));
				MAP *tile = *MapPtr + tileIndex;
...
void populateTileInfos()
{
	...
	for (int tileIndex = 0; tileIndex < *map_area_tiles; tileIndex++)
	{
		...
		tileInfo.bunker = map_has_item(tile, TERRA_BUNKER);
...
int getGravshipUnitTravelTime(MAP *origin, MAP *destination, int speed, int action)
{
...
*/
int getAirRangedUnitTravelTime(MAP *origin, MAP *destination, int speed, int action)
{
	assert(origin >= *MapPtr && origin < *MapPtr + *map_area_tiles);
	assert(destination >= *MapPtr && destination < *MapPtr + *map_area_tiles);
	...
	for (MAP *airbase : aiData.factionInfos[aiFactionId].airbases)
	{
		...
		if (airbaseToTargetRange + action > movementSpeed)
			continue;
...
MAP *getSafeLocation(int vehicleId, int baseRange)
{
	...
	for (MAP *tile : getRangeTiles(vehicleTile, baseRange, true))
	{
		...
		if ((triad == TRIAD_LAND || triad == TRIAD_SEA) && map_has_item(tile, TERRA_BUNKER) && !tileFactionInfo.blocked[0])
		{
	...
	for (MAP *tile : getRangeTiles(vehicleTile, MAX_SAFE_LOCATION_SEARCH_RANGE, true))
	{
		...
		else if ((triad == TRIAD_LAND || triad == TRIAD_SEA) && map_has_item(tile, TERRA_BUNKER) && !tileFactionInfo.blocked[0])
		{
			// not blocked bunker is a safe location for surface vehicle
		}
		else
...
void populateHexCosts()
{
	...
	for (MAP *tile = *MapPtr; tile < *MapPtr + *map_area_tiles; tile++)
	{
		...
		for (unsigned int angle = 0; angle < ANGLE_COUNT; angle++)
		{
			...
			tileInfo.hexCosts[MT_AIR][angle] = Rules->mov_rate_along_roads;
			...
			if (!tileOcean && !adjacentTileOcean)
			{
				...
				if (!map_has_item(tile, TERRA_FUNGUS) && regularHexCost >= 2 * Rules->mov_rate_along_roads)
				{
					regularHexCost = divideIntegerRoundUp(Rules->mov_rate_along_roads * 5, 4);
				}
				...
				{
					easyHexCost = divideIntegerRoundUp(Rules->mov_rate_along_roads * 5, 4);
				}
