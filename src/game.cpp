...
int faction_might(int faction) {
    Faction* f = &Factions[faction];
    return std::max(1, f->mil_strength_1 + f->mil_strength_2 + f->pop_total);
}
...
int min_range(const Points& S, int x, int y) {
    int z = MaxMapW;
    for (auto& p : S) {
        z = std::min(z, map_range(x, y, p.x, p.y));
    }
    return z;
}
...
*/
// =WTP=
// add parameter to return first tile
MAP* TileSearch::get_next(bool returnFirstTile) {
    while (head < tail) {
        ...
        // =WTP=
        // return first tile if instructed
//        if (!first) {
        if (!first || returnFirstTile) {
            return sq;
        }
    }
    return NULL;
