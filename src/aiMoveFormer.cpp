		...
		for (MAP *tile : getBaseRadiusTiles(base->x, base->y, false))
		{
			...
			if (!is_ocean(tile))
			{
				if (map_rockiness(tile) == 2)
				{
					...
					if (!map_has_item(tile, TERRA_MINE) || tileTerraformingInfo.workedBaseId == -1)
					{
				...
				{
					if (map_has_item(tile, TERRA_FOREST) && tileTerraformingInfo.workedBaseId == -1)
					{
						// potential source of minerals
						baseTerraformingInfo.unusedMineralTileCount++;
					}
...
ConventionalTerraformingRequest calculateConventionalTerraformingScore(int baseId, MAP *tile, TERRAFORMING_OPTION *option, bool fitness)
{
	...
	for(int action : option->actions)
	{
		...
		if (optionFungus && isRemoveFungusRequired(action))
		{
			...
			if (!rocky)
			{
				int nearbyForestKelpCount = nearby_items(getX(tile), getY(tile), 1, (ocean ? TERRA_FARM : TERRA_FOREST));
		...
		if (ocean)
		{
			...
			{
				int nearbyKelpCount = nearby_items(getX(tile), getY(tile), 1, TERRA_FARM);
		...
		else
		{
			...
			{
				int nearbyForestCount = nearby_items(getX(tile), getY(tile), 1, TERRA_FOREST);
	...
	{
		if (map_has_item(tile, TERRA_FARM))
		{
			if (!map_has_item(tile, TERRA_MINE | TERRA_SOLAR | TERRA_CONDENSER | TERRA_ECH_MIRROR))
			{
...
double calculateSensorScore(MAP *tile)
{
	...
	for (MAP *rangeTile : getRangeTiles(tile, *map_half_x - 1, false))
	{
		// detect not own tile
...
double calculateFitnessScore(MAP *tile, int action, bool levelTerrain)
{
	...
	if (ocean)
	{
		...
		case FORMER_SENSOR:
			{
				if (tileTerraformingInfo.workedBaseId != -1 || map_has_item(tile, TERRA_MINE | TERRA_SOLAR))
				{
...
double estimateCondenserExtraYieldScore(MAP *improvedTile, const std::vector<int> *actions)
{
	...
	for (MAP *boxTile : getRangeTiles(tile, 1, true))
	{
		...
		if (map_has_item(boxTile, TERRA_THERMAL_BORE))
			continue;
