...
//	}
    ...
    else if (MATCH("wtp", "alternative_tech_cost"))
    {
    ...
    else if (MATCH("wtp", "energy_market_crash_numerator"))
    {
    ...
    else if (MATCH("wtp", "ai_production_facility_priority"))
    {
        cf->ai_production_facility_priority = atof(value);
    }
...
int find_facility(int id) {
    ...
    for (const int* item : build_order) {
        ...
        if (item[1] & F_Combat && m->thinker_enemy_range > 40 - std::min(16, 4*fc->maint))
            continue;
...
bool need_scouts(int x, int y, int faction, int scouts) {
    ...
    int score = std::max(-2, 5 - *current_turn/10)
        + 2*(*map_native_lifeforms) - 3*scouts
        + std::min(8, f->region_territory_goodies[sq->region] / 4);
...
int select_production(const int id) {
    ...
    double w1 = std::min(1.0, std::max(0.5, 1.0 * minerals / p->proj_limit))
        ...
        * std::min(1.0, std::max(0.3, (*current_turn/40.0)));
    ...
    int crawl_target = std::min(1 + base->pop_size/4,
        (minerals >= p->proj_limit ? (bases < 3 ? 2 : 1) : 0) + std::max(0, 3 - bases/2));
