...
This is to prevent excessive total population growth at very early stages. It also sets the limit to **less** than average to make sure the benefit automatically wears off when a lot of new bases created. Player needs to develop the rest of their bases to reap the maximum benefit.

### The Cloning Vats

Permanent population boom in all bases is one of the most game breaking effects in entire SMACX. Modders struggle to mitigate and balance it. WTP moderates this project effect to the level comparable to others.

The Cloning Vats grants +2 GROWTH bonus instead of triggering population boom. It also does not grant impunities anymore.
...
* Probe stays on its square and subverted unit is moved to the probe square.
* Probe loses one turns per subversion action.
...
* Vector distance to HQ coefficient
	* 1.0 if no HQ or more than than the quarter of the map width
	* 2.0 if at HQ
	* linearly prorated for anything in between
...
* Distance to HQ is still a factor but it works in more smooth and sane way, though. Like absence of HQ does not increase cost in some case as in vanilla. Also bases next to HQ will not have exorbitant cost anymore.
...
Actual base MC also includes subversion cost of all target faction units at or adjacent to base. All non target faction units in base after MC are destroyed after MC.
