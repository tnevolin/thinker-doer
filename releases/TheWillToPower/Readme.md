...
Building sensors to extend protection bonus is nice idea but they are easy destroyable with single artillery shot or with Needlejet raid. They are so easy target that it is considered stupid to storm base without destroying them fist. Therefore, their bonus value doesn't matter at all. We can make them confer 100% bonus -> it is still equal to zero when they are gone. Consequently, I've decided to remove their defense bonus altogether and keep them as observation stations which is still quite useful tool for human player.

Summarizing the above, homeland combat bonus is a nice replacement for Children Creche and Sensors combat bonuses but more logical, simpler, more understandable, move visualizable. It is also displays properly in combat calculation screen with own explicit name and bonus. Essentially the rule is that troops fight 50% stronger on mother land. Neat! How simple and logical is that?
